...
public class Pentomino {
    ...
    public void checkMovementCollision() {
        ...
        for (int i = 0; i < 5; i++) {
            if (b[i].x == GamePanel.left_x) {
                leftCollision = true;
            }
            if (b[i].x >= GamePanel.right_x - Block.SIZE) {
                rightCollision = true;
            }
            if (b[i].y >= GamePanel.bottom_y - Block.SIZE) {
    ...
    public void checkRotationCollision() {
        ...
        for (Block block : tempB) {
            if (block.x + Block.SIZE > GamePanel.right_x) {
                rightCollision = true;
                break;
            }
        }
        for (Block block : tempB) {
            if (block.y + Block.SIZE > GamePanel.bottom_y) {
                bottomCollision = true;
                break;
            }
        }
    ...
    private void checkStaticCollision() {
        for(int i = 0; i< GamePanel.staticBlocks.size(); i++) {
            int staticX = GamePanel.staticBlocks.get(i).x;
            int staticY = GamePanel.staticBlocks.get(i).y;
            for (int j = 0; j < 5; j++) {
